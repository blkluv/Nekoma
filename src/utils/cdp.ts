...
import { toAccount, Account } from "viem/accounts";
...
export async function createServerWalletForUser(
  ...
  try {
    ...
    const existingWallet = serverWallets.get(userAddress);
    if (existingWallet?.smartAccount) {
    ...
    const walletClient = createWalletClient({
      account: toAccount(account) as Account,
      chain: base,
      transport: http(),
    });
